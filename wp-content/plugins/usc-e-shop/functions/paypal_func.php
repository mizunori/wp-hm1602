...
jQuery(function($) {
	paypalfunc = {
		...
		usePoint: function() {
			var s = this.settings;
			s.data = "usces_ajax_action=paypal_use_point&usepoint=" + $("#set_usedpoint").val() + "&total_price=<?php esc_html_e($total_price); ?>&item_price=<?php esc_html_e($item_price); ?>&tax=<?php esc_html_e($tax); ?>";
			s.success = function( res, dataType ) {
				var r = res.split( "#usces#" );
				if( r[0] == "error" ) {
