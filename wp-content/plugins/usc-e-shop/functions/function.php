...
//			$optstr = implode(',', $options);
	...
	if( $_POST['mode'] == 'completionMail' ){
		$tracking_number = $usces->get_order_meta_value( 'tracking_number', $order_id );
...
//	$msg_body .= usces_mail_line( 2, $data['order_email'] )."\r\n";//--------------------
	...
	if( $usces->options['put_customer_name'] == 1 ) {
		$name = sprintf( __('Mr/Mrs %s', 'usces'), usces_localized_name(trim($data['order_name1']), trim($data['order_name2']), 'return') );
		$message = $name."\r\n\r\n".$message;
	}
...
//			$optstr = implode(',', $options);
	...
	$name = sprintf( __('Mr/Mrs %s', 'usces'), usces_localized_name(trim($entry["customer"]["name1"]), trim($entry["customer"]["name2"]), 'return') );
...
function usces_send_inquirymail() {
	...
	if( $usces->options['put_customer_name'] == 1 ) {
		$message = sprintf(__('Mr/Mrs %s', 'usces'), $inq_name)."\r\n\r\n".$message;
	}
	...
	if ( $res0 ) {
		$subject = apply_filters( 'usces_filter_inquiry_subject_to_manager', __('** An inquiry **','usces').'('.sprintf(__('Mr/Mrs %s', 'usces'), $inq_name).')',$mats);
...
function usces_send_regmembermail($user) {
	...
	$name = sprintf( __('Mr/Mrs %s', 'usces'), usces_localized_name(trim($user['name1']), trim($user['name2']), 'return') );
...
function usces_clearup_acting_log() {
	...
	//if( $settlement_backup == 1 ) {
		...
		$query = $wpdb->prepare("DELETE FROM {$table} WHERE log_type = %s AND datetime < %s", 'acting_data', $date );
		$wpdb->query($query);
	//}
}
