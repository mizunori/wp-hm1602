...
function usces_add_tracking_number_field($data, $cscs_meta, $action_args){
	...
	$tracking_number = '';
	if( !empty($data['ID']) ){
		$tracking_number = $usces->get_order_meta_value('tracking_number', $data['ID']);
	...
	$res = '
			<tr>
				...
				<td class="col1">
					<input name="delivery_company" type="text" style="width:100%;" value="' . esc_attr($delivery_company). '">
				</td>
			</tr>
			<tr>
...
function usces_update_tracking_number($new_orderdata){
	...
	if( isset($_POST['tracking_number']))
		$usces->set_order_meta_value('tracking_number', $_POST['tracking_number'], $new_orderdata->ID);
...
function usces_instance_extentions(){
	global $ganbare_tencho, $atobarai_com, $sagawa_express, $kuroneko_yamato;
	
	require_once(USCES_EXTENSIONS_DIR."/GanbareTencho/GanbareTencho.class.php");
	$ganbare_tencho = new USCES_GANBARE_TENCHO();
//	require_once(USCES_EXTENSIONS_DIR."/AtobaraiComCSV/AtobaraiComCSV.class.php");
//	$atobarai_com = new USCES_ATOBARAICOM_CSV();
}
