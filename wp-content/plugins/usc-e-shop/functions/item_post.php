			...
			<tr>
				<td colspan='2' class='submittd'>
					<div id='itemoptsubmit-<?php echo $id; ?>' class='submit'>
						<input name='deleteitemopt[<?php echo $id; ?>]' id='deleteitemopt[<?php echo $id; ?>]' type='button' class='button' value='<?php esc_attr_e(__( 'Delete' )); ?>' onclick="if( jQuery('#post_ID').val() < 0 ) return; itemOpt.post('deleteitemopt', <?php echo $id; ?>);" />
						<input name='updateitemopt[<?php echo $id; ?>]' id='updateitemopt[<?php echo $id; ?>]' type='button' class='button' value='<?php esc_attr_e(__( 'Update' )); ?>' onclick="if( jQuery('#post_ID').val() < 0 ) return; itemOpt.post('updateitemopt', <?php echo $id; ?>);" />
			...
			<tr>
				<td colspan='5' class='submittd'>
					<div id='skusubmit-<?php echo $id; ?>' class='submit'>
						<input name='deleteitemsku[<?php echo $id; ?>]' id='deleteitemsku[<?php echo $id; ?>]' type='button' class='button' value='<?php esc_attr_e(__( 'Delete' )) ?>' onclick="if( jQuery('#post_ID').val() < 0 ) return; itemSku.post('deleteitemsku', <?php echo $id; ?>);" />
						<input name='updateitemsku[<?php echo $id; ?>]' id='updateitemsku[<?php echo $style; ?>]' type='button' class='button' value='<?php esc_attr_e(__( 'Update' )); ?>' onclick="if( jQuery('#post_ID').val() < 0 ) return; itemSku.post('updateitemsku', <?php echo $id; ?>);" />
...
function order_item_ajax(){
	...
	switch ( $_POST['mode'] ) {
		...
		case 'delete_settlement_log':
			$res = usces_delete_settlement_log( $_POST['log_key'] );
			break;
		case 'delete_settlement_log_all':
			$res = usces_delete_settlement_log();
			break;
...
function usces_get_settlement_log() {
	global $usces, $wpdb;
	...
	foreach( (array)$log_data as $data ) {
		...
		$class = '';
		$res .= '<tr'.$class.'>
			<td><input type="checkbox" class="log-check" id="log_key_'.$data['log_key'].'"></td>
			<td><input type="button" class="log-detail button" id="'.$data['log_key'].'" value="'.__('Detail','usces').'"></td>
			<td>'.$data['datetime'].'</td>
			<td>'.$data['log_key'].'</td>
			<td>'.$name.'</td>
			<td>'.$payment_name.$payment_structure.'</td>
		</tr>';
	}
...
function usces_get_settlement_log_detail( $log_key ) {
	...
	$html = '';
	$html .= '<tr><th>'.__('Register date','usces').'</th><td>'.$order_data['datetime'].'<label>
		<span><input type="radio" id="register_date_0" value="0" checked="checked" /><span>'.__('Set the Registered Date to the Order Date.','usces').'</span></label><br />
		<input type="radio" id="register_date_1" value="1" /><span>'.__('Set the current time to the Order Date.','usces').'</span></label></span></td></tr>';
	...
	foreach( $order_data['usces_cart'] as $serial => $row ) {
		...
		$cart_item_name = $usces->getCartItemName_byOrder($row);
	...
	if( usces_is_tax_display() && 'products' == usces_get_tax_target() ) {
		$html .= '<tr><th colspan="3">'.usces_tax_label(array(), 'return').'</th><td>'.usces_tax($usces_entries, 'return').'</td></tr>';
	}
