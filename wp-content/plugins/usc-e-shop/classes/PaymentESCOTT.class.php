...
//			add_action( 'plugins_loaded', array( $this, 'conv_result_notification') );
	...
	***********************************************/
	public function is_complete_settlement( $complete, $payment_name, $status ){
		...
		if( 'acting_escott_card' == $payments[$payment_name]['settlement'] ){
			$complete = true;
		}
		return $complete;
	}
...
usces_log('Conv test : '.print_r($_REQUEST, true), 'acting_transaction.log');
	...
	public function error_page_mesage( $res ){
		...
		if( isset($_REQUEST['MerchantFree2']) && 'acting_escott_card' == $_REQUEST['MerchantFree2'] ){
			...
			case 'G65':
				...
				break;
			default:
				if( usces_get_order_id_by_trans_id( (int)$_REQUEST['MerchantFree1'] ) ){
					$res .= '<div class="error_page_mesage">
					<p>ご注文は既に完了いたしております。</p>
					<p>このページを再表示しないようお願いいたします。</p>
					</div>';
				}else{
					$res .= '<div class="error_page_mesage">
	...
	* @return boolean $res
	***********************************************/
	public function is_activate_card( $paymod_base ){
		global $usces;
		
		if( isset($usces->options['acting_settings'][$paymod_base]) 
			&& 'on' == $usces->options['acting_settings'][$paymod_base]['card_activate'] 
	...
	***********************************************/
	public function point_inform(){
		...
		$securecode = isset($_POST['securecode']) ? $_POST['securecode'] : '';
	?>
		<input type="hidden" name="cardnumber" value="<?php esc_attr_e($_POST['cnum1']) ?>">
		<input type="hidden" name="securecode" value="<?php esc_attr_e($securecode) ?>">
		<input type="hidden" name="expyy" value="<?php esc_attr_e($_POST['expyy']) ?>">
		<input type="hidden" name="expmm" value="<?php esc_attr_e($_POST['expmm']) ?>">
		<input type="hidden" name="username" value="<?php esc_attr_e($_POST['username_card']) ?>">
		<input type="hidden" name="cnum1" value="<?php esc_attr_e($_POST['cnum1']) ?>">
		<input type="hidden" name="username_card" value="<?php esc_attr_e($_POST['username_card']) ?>">
	<?php
	}
...
?>
	...
	<form action="" method="post" name="escott_form" id="escott_form">
		...
		<table class="settle_table">
			...
			<tr>
				<th>支払制限日数</th>
				<td colspan="3"><input name="conv_limit" type="text" id="conv_limit" value="<?php esc_html_e(isset($opts['escott']['conv_limit']) ? $opts['escott']['conv_limit'] : '7'); ?>" />日</td>
				<td></td>
			</tr>
		</table>
...
{ 
	...
	{ 
		...
		if( empty($param_list) === false ){ 
			...
			// ソケット通信接続 
			$fp = @fsockopen( 'tls://'.$url['host'], 443, $errno, $errstr, $this->connection_timeout ); 
			if( $fp === false ){ 
				usces_log('e-SCOTT send error : '."TLS接続に失敗しました", 'acting_transaction.log');
