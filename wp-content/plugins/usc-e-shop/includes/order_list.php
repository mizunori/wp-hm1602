...
//	});
	...
	operation = {
		...
		get_settlement_log: function() {
			...
			$.ajax({
				...
				if( data[0] == "OK" ) {
					$("#settlement-log-list tbody").html(data[1]);
				}
		...
		get_settlement_log_detail: function( log_key ) {
			$.ajax({
				...
				var data = retVal.split("#usces#");
				if( data[0] == "OK" ) {
					$("#settlement-log-detail tbody").html(data[1]);
				}
			}).fail(function( retVal ) {
			});
			return false;
		...
		delete_settlement_log: function() {
		},
		...
		revival_settlement_data: function( log_key ) {
			$.ajax({
				url: ajaxurl,
				type: "POST",
				data: {
					action: "order_item_ajax",
					mode: "revival_settlement_data",
					log_key: log_key
				}
			}).done(function( retVal, dataType ) {
				var data = retVal.split("#usces#");
		...
		buttons: {
			"<?php _e('Clear log', 'usces'); ?>": function() {
				if( confirm("<?php _e('Are you sure you want to delete all log ?', 'usces'); ?>") ) {
					//operation.delete_settlement_log_all();
					$("#settlement-log-list tbody").html("");
				}
			},
		...
		buttons: {
			"<?php _e('Create order data', 'usces'); ?>": function() {
				if( confirm("<?php _e('Are you sure you want to create the order data ?', 'usces'); ?>") ) {
					operation.revival_settlement_data( $("#log_key").val() );
				}
			},
			"<?php _e('Close'); ?>": function() {
				$(this).dialog("close");
	...
	$(document).on( "click", ".log-detail", function() {
		...
		$("#settlement-log-detail tbody").html("");
		operation.get_settlement_log_detail( key );
...
<?php endforeach; ?>
		...
		<td><input type="button" id="settlementlog" class="searchbutton" value="<?php _e('Settlement Log List', 'usces'); ?>" /></td>
		<?php endif; ?>
		</tr>
		</table>
...
?>
	<tr<?php echo apply_filters('usces_filter_order_list_detail_trclass', '', $array); ?>>
		<?php echo apply_filters('usces_filter_order_list_detail', $list_detail, $array, $curent_url); ?>
	</tr>
<?php endforeach; ?>
</table>
...
<div id="settlementLogDialog" title="<?php _e('Settlement Log List', 'usces'); ?>">
	<div id="settlement-log-response"></div>
	<fieldset>
		<table id="settlement-log-list">
		<tbody>
		</tbody>
		</table>
	</fieldset>
</div>
<div id="settlementLogDetailDialog" title="<?php _e('Settlement Log Detail', 'usces'); ?>">
	<fieldset>
		<table id="settlement-log-detail">
		<tbody>
		</tbody>
		</table>
	</fieldset>
